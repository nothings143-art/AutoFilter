...
BOT_TOKEN = environ.get('BOT_TOKEN', "8338749287:AAHnPoh20fDJVkvHsH6VeJNjLoTAy-27B_w")
...
# ============================
ADMINS = [int(admin) if id_pattern.search(admin) else admin for admin in environ.get('ADMINS', '8210377618').split()] # Replace with the actual admin ID(s) to add
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1002968244951').split()]  # Channel id for auto indexing (make sure bot is admin)
LOG_CHANNEL = int(environ.get('LOG_CHANNEL', '-1002998728405'))  # Log channel id (make sure bot is admin)
BIN_CHANNEL = int(environ.get('BIN_CHANNEL', '-1003007412566'))  # Bin channel id (make sure bot is admin)
MOVIE_UPDATE_CHANNEL = int(environ.get('MOVIE_UPDATE_CHANNEL', '-1002658359941'))  # Notification of those who verify will be sent to your channel
PREMIUM_LOGS = int(environ.get('PREMIUM_LOGS', '-1001868871195'))  # Premium logs channel id
auth_channel = environ.get('AUTH_CHANNEL', '-1002968244951')  # Channel/Group ID for force sub (make sure bot is admin)
...
AUTH_CHANNEL = [int(fch) if id_pattern.search(fch) else fch for fch in environ.get('AUTH_CHANNEL', '-1002968244951').split()]
MULTI_FSUB = [int(channel_id) for channel_id in environ.get('MULTI_FSUB', '-1002968244951').split() if re.match(r'^-?\d+$', channel_id)]  # Channel for force sub (make sure bot is admin)
